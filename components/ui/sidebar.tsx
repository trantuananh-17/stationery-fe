...
function SidebarTrigger({
  ...
  const shouldShowLabel = withLabel && (isMobile || state === 'expanded');
  ...
  return (
    <Button
      ...
      variant='ghost'
      size='default'
      className={cn('w-auto justify-start gap-2 px-2', className)}
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
      }}
    ...
    >
      ...
      {shouldShowLabel && <span className='text-sm font-medium whitespace-nowrap'>{label}</span>}
